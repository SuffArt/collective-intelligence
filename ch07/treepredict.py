...
lines = [line.split('\t') for line in file('decision_tree_example.txt')]
for line in lines:
...
def divideset(rows,column,value):
	...
	if isinstance(value,int) or isinstance(value,float):
		split_function=lambda row: float(row[column])>=value
	else:
...
def buildtree(rows,scoref=entropy):
	...
	best_criteria=None
	best_sets=None
	column_count=len(rows[0])-1 # The number os attributes (excluding the result in the last row)
